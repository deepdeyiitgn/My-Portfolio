...
export default function Footer() {
  ...
  const qlOverlayRef = useRef<HTMLDivElement>(null);
  const deepOverlayRef = useRef<HTMLDivElement>(null);
    ...
    <footer className="w-full border-t border-zinc-900 bg-zinc-950/80 backdrop-blur-md pt-24 pb-12 px-6 overflow-hidden relative">
      ...
      <div className="max-w-7xl xl:max-w-screen-2xl 2xl:max-w-[1800px] mx-auto space-y-20">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 py-2">
          ...
          >
            ...
            <div className="flex flex-col items-start leading-tight">
              <span className="text-[15px] font-semibold text-zinc-300">
                Powered by QuickLink<span className="text-[9px] align-super">™</span>
              </span>
              <span className="text-[12px] text-zinc-500">#1 URL Shortening Platform</span>
            </div>
          </button>
          ...
          >
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[12px] text-zinc-500">AI &amp; Web Development Solutions</span>
            </div>
          </button>
        </div>
        ...
        {qlOpen && (
          <div
            ref={qlOverlayRef}
            className="fixed inset-0 flex items-center justify-center z-[9999] bg-black/55 backdrop-blur-[4px]"
            onClick={(e) => { if (e.target === qlOverlayRef.current) setQlOpen(false); }}
          >
            <div className="relative w-[90%] max-w-[480px] rounded-3xl text-center shadow-[0_40px_100px_rgba(0,0,0,.6)]"
              style={{ background: '#18181b', padding: '38px 30px 30px' }}>
              <button
                onClick={() => setQlOpen(false)}
                className="absolute top-[18px] right-[20px] bg-transparent border-none text-[20px] cursor-pointer text-zinc-500 hover:text-zinc-100 transition-colors"
              >✕</button>
              {/* Header */}
              <div className="mb-4">
                <img src="https://qlynk.vercel.app/quicklink-logo.svg" alt="QuickLink" className="h-12 mx-auto mb-3.5" />
                <h2 className="m-0 text-[22px] font-bold text-zinc-100">QuickLink<span className="text-[9px] align-super">™</span></h2>
                <p className="text-[14px] text-zinc-400 mt-1.5">Smart URL &amp; QR Platform</p>
              </div>
              {/* Body */}
              <div className="text-left">
                <p className="text-[14px] text-zinc-400 leading-[1.7] my-3.5">QuickLink is an all-in-one smart link shortening and QR platform designed for speed, clarity and control.</p>
                <p className="text-[14px] text-zinc-400 leading-[1.7] my-3.5">Create short links, generate dynamic QR codes, manage redirects, and track performance — all in one place.</p>
                <p className="text-[14px] text-zinc-400 leading-[1.7] my-3.5">Built for creators, developers, students and modern businesses who value simplicity and efficiency.</p>
                <p className="text-[14px] text-zinc-300 mt-2.5"><strong>Founder &amp; Creator:</strong> Deep Dey</p>
              </div>
              {/* Buttons */}
                ...
                {[
                  {
                    ...
                    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>,
                  },
                  {
                    url: 'https://github.com/deepdeyiitgn',
                    label: 'GitHub',
                    style: { background: 'linear-gradient(135deg,#1e293b,#334155)' },
                    icon: <svg viewBox="0 0 24 24" width="18" height="18"><path d="M12 2C6.5 2 2 6.6 2 12.2c0 4.4 2.9 8.2 6.9 9.5.5.1.7-.2.7-.5v-1.9c-2.8.6-3.4-1.4-3.4-1.4-.4-1.1-1-1.4-1-1.4-.8-.6.1-.6.1-.6.9.1 1.3.9 1.3.9.8 1.4 2 1 2.6.8.1-.6.3-1 .6-1.3-2.2-.3-4.5-1.2-4.5-5.2 0-1.1.4-2 1-2.8-.1-.3-.4-1.3.1-2.7 0 0 .8-.3 2.8 1 .8-.2 1.6-.3 2.4-.3s1.6.1 2.4.3c2-.1 2.8-1 2.8-1 .5 1.4.2 2.4.1 2.7.6.8 1 1.7 1 2.8 0 4-2.3 4.9-4.5 5.2.3.3.6.9.6 1.8v2.7c0 .3.2.6.7.5 4-1.3 6.9-5.1 6.9-9.5C22 6.6 17.5 2 12 2z" fill="white"/></svg>,
                  },
                  {
                    url: 'https://youtube.com/channel/UCrh1Mx5CTTbbkgW5O6iS2Tw/',
                    label: 'YouTube',
                    style: { background: 'linear-gradient(135deg,#ef4444,#db2777)' },
                    icon: <svg viewBox="0 0 24 24" width="18" height="18"><path d="M23 12s0-3.5-.4-5c-.2-.8-.8-1.4-1.6-1.6C19.5 5 12 5 12 5s-7.5 0-9 .4c-.8.2-1.4.8-1.6 1.6C1 8.5 1 12 1 12s0 3.5.4 5c.2.8.8 1.4 1.6 1.6 1.5.4 9 .4 9 .4s7.5 0 9-.4c.8-.2 1.4-.8 1.6-1.6.4-1.5.4-5 .4-5zM10 15V9l6 3-6 3z" fill="white"/></svg>,
                  },
                  {
                    url: 'https://clock.qlynk.me',
                    label: 'Study Clock',
                    style: { background: 'linear-gradient(135deg,#6366f1,#8b5cf6)' },
                    icon: <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>,
                  },
                  ...
                  <button
                    ...
                    style={style}
                    className="flex items-center justify-center gap-2.5 py-4 border-none rounded-2xl text-[15px] font-semibold cursor-pointer text-white hover:-translate-y-1 hover:shadow-[0_14px_30px_rgba(0,0,0,.4)] transition-all"
                  >
                    {icon}{label}
                  </button>
                ))}
        ...
        {deepOpen && (
          <div
            ref={deepOverlayRef}
            className="fixed inset-0 flex items-center justify-center z-[10000] bg-black/60 backdrop-blur-[5px] px-5"
            onClick={(e) => { if (e.target === deepOverlayRef.current) setDeepOpen(false); }}
          >
            <div className="relative w-full max-w-[380px] rounded-[20px] text-center shadow-[0_20px_40px_rgba(0,0,0,.7)]"
              style={{ background: '#18181b', padding: '35px 25px' }}>
              <button
                onClick={() => setDeepOpen(false)}
                className="absolute top-[15px] right-[20px] text-[28px] font-bold cursor-pointer bg-transparent border-none text-zinc-500 hover:text-zinc-100 transition-colors leading-none"
              >&times;</button>
              <img
                src="https://qlynk.me/wiki-images/Deep_Dey_New.png"
                alt="Deep Dey"
                className="w-[90px] h-[90px] rounded-full object-cover border-[3px] border-zinc-700 shadow-[0_0_20px_rgba(245,158,11,0.4)] mb-4 mx-auto"
              />
              <div className="text-[22px] font-extrabold mb-2.5 text-zinc-100">Hello, I&apos;m Deep!</div>
              <p className="text-[14px] text-zinc-400 mb-6 leading-[1.6]">
                ...
                <br /><br />
                I architect <span className="text-amber-500 font-bold">high-performance digital ecosystems</span> —
              ...
              <div className="flex gap-2 justify-center flex-wrap mb-2.5">
                {[
                  { key: 'wiki', label: 'More Info', style: { background: 'linear-gradient(135deg,#27272a,#52525b)' }, url: 'https://qlynk.vercel.app/wiki' },
                  ...
                  <button
                    ...
                    onClick={() => window.open(url, '_blank', 'noopener,noreferrer')}
                    style={style}
                    className="inline-flex items-center justify-center text-white px-5 py-2.5 rounded-full text-[13px] font-semibold shadow-md hover:-translate-y-0.5 hover:shadow-lg transition-all min-w-[110px] cursor-pointer border-none"
                  >{label}</button>
                ))}
              </div>
